...
def save_bible_progress(user_id, book, chapter, verse):
    ...
    # Check if the user already has a current progress record.
    cursor.execute(
        """
        SELECT id
        FROM bible_progress
    ...
    # Update existing current progress.
    ...
    # Create first current progress record.
    else:
        cursor.execute(
            """
    ...
    # Keep a separate record of every update for the history page.
    cursor.execute(
        """
...
def get_bible_history_paginated(user_id, page=1, per_page=10):
    ...
    # Get one page of history records.
    cursor.execute(
        """
        SELECT book, chapter, verse, saved_at
        FROM bible_progress_history
...
def get_total_verses(book, chapter):
    ...
    # Return total verses if found, otherwise return None.
...
def get_group_devotionals_for_user(user_id):
    ...
    # Return an empty list if the user is not in any group.
    if not group_ids:
        return []
...
def can_user_edit_delete(user_id, devotional_id):
    ...
    # Return False if the devotional does not exist.
    if not devotional:
        conn.close()
        return False
    ...
    # Admin can edit and delete any devotional.
    if role == "admin":
        return True

    # Leader can edit and delete their own devotionals.
...
def get_approved_threads(user_id):
    ...
    # If user belongs to groups, return global and group threads.
    if group_ids:
        placeholders = ",".join("?" for _ in group_ids)
    ...
    # If user has no groups, return only global threads.
    else:
        cursor.execute(
            """
            SELECT *
...
def get_all_gatherings():
    """
    Return only upcoming gatherings.
    """
    conn = connect_db()
    cursor = conn.cursor()
...
def can_edit_gathering(user_role, user_id, gathering_creator_id):
    ...
    """
    # Admin can edit or delete any gathering.
    if user_role == "admin":
        return True

    # Leader can edit or delete gatherings they created.
...
def can_manage_group(user_id, user_role, group_id):
    ...
    # Leaders can only manage groups where they are the leader.
    cursor.execute(
        """
        SELECT id
        FROM groups
...
def get_prayer_requests_paginated(page=1, per_page=5):
    ...
    # Get one page of approved prayer requests.
    cursor.execute(
        """
        SELECT request_id, request_text, submitted_at
        FROM prayer_requests
    ...
    # Count total approved requests for pagination.
    cursor.execute(
        """
        SELECT COUNT(*)
        FROM prayer_requests
...
def get_latest_user_devotional(user_id):
    ...
    # Create placeholders if the user belongs to groups.
    placeholders = ",".join("?" for _ in user_group_ids) if user_group_ids else ""

    # Build query to return global devotionals and user's group devotionals.
    query = f"""
        SELECT *
        FROM devotionals
